...
std::pair<arma::vec, arma::vec> ProgressiveStatistics(string flag, Random &rnd, int N, int L, double l, double d){

    if (flag != "averages" && flag != "uncertainty" && flag!= "buffon"){
    ...
    for (int j = 0; j < N; j++){
        ...
        if (flag == "averages"){
            ...
            for (int i = 0; i < L; i++){
                ...
                double bar = rnd.Rannyu()*d;
                if ((bar + (l / 2) * sin(theta)) >= d || (bar - (l / 2) * sin(theta)) <= 0){
                    temp_average += 1;
                }
            }
        ...
        if (flag == "buffon"){
            temp_average = ((2*l)/(temp_average*d));
        }
