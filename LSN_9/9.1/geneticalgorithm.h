...
class Generation  {
    ...
    */
    ...
    public:
        Generation(Random *rnd, int M, double probSwapMutation, double probShiftMutation, double probPermutationMutation, double probInversionMutation, double probCrossover, double p); // empty constructor
        ~Generation() { delete m_chromo; };         // empty destructor

        void CreateInitialPopulation();
        void UpdateFitness();                       //Update the fitness of a Generation
        double GetOptimumLoss() {return m_fitness.at(0, 0); };
        int GetNumberOfCities() {return m_chromo->GetNumberOfGenes() - 1;};
        ...
        //Test
        arma::vec Test() { return m_chromo->NewChromosome(); };

        //Operators
        int Selection() {return int(m_M * pow(m_rnd->Rannyu(), m_p));};                // Loaded die      
        void CrossOver();
        void Mutations(int index);
